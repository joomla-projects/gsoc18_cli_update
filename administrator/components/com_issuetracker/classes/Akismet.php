 ...
 *
 * Modified by G S Chapman to suit our email spam checks.
 * Added user agent override and add some extra checks that $_SERVER variables are present.
 * December 2013
 */
...
class Akismet
{
   ...
   public function __construct($blogURL, $wordPressAPIKey) {
      ...
      if(isset($_SERVER['HTTP_USER_AGENT'])) {             // GSC added to check email.
       ...
       * PHP5 one...
       */
      if(isset($_SERVER['REMOTE_ADDR'])) {        // GSC Added to check email.
   ...
   /**
    * To override the references field when submitting spam/ham later on
    *
    * @param string $references   An IP address.  Optional.
    */
   public function setReferences($references) {
      // $this->comment['references'] = $references;
   }

   /**
    * To override the in_reply_to field when submitting spam/ham later on
    *
    * @param string $in_reply_to   An IP address.  Optional.
    */
   public function setInreplyto($in_reply_to) {
      // $this->comment['in_reply_to'] = $in_reply_to;
   }
   ...
   /**
    * To override the user agent when submitting spam/ham later on
    *
    * @param string $useragent   A User agent.  Optional.
    */
...
class SocketWriteRead
{
    ...
    *
    * @throws  Exception
    * @exception 'Error connecting to host: ' An exception is thrown if a connection cannot be made to the remote host.
    */
