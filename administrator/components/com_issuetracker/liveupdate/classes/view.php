...
class LiveUpdateView extends JoomlaCompatView
{
	public function display($tpl = null)
	{
		...
		if(!$config->addMedia()) {
			...
			$url = JURI::base().'/components/'.JFactory::getApplication()->input->get('option','').'/liveupdate/assets/liveupdate.css';
			$document->addStyleSheet($url, 'text/css');
		}

		$requeryURL = rtrim(JURI::base(),'/').'/index.php?option='.JFactory::getApplication()->input->get('option','').'&view='.JFactory::getApplication()->input->get('view','liveupdate').'&force=1';
