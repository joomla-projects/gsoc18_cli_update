<?php
/**
 * @Version       $Id: projectedit.php 2167 2016-01-01 16:41:39Z geoffc $
...
/**
 * Class JFormFieldProjectEdit
 */
class JFormFieldProjectEdit extends JFormFieldList
{
   /**
    ...
    * @since   1.6
    */
   public $type = 'ProjectEdit';

   /**
    * Method to get a list of projects that respects access controls and can be used for
    * either project assignment or parent project assignment in edit screens.
    * Use the parent element to indicate that the field will be used for assigning parent projects.
    *
   ...
   protected function getOptions()
   {
      ...
      // Load the project options for a given extension.

      // For projectss the old projectry is the project id or 0 for new project.
      if ($this->element['parent'] || $jinput->get('option') == 'com_issuetracker') {
      ...
      if ($this->element['parent'] == true || $jinput->get('option') == 'com_issuetracker')  {
...
//       $query->where('(a.extension = '.$db->quote($extension).')');
      ...
      if ($oldCat != 0 && ($this->element['parent'] == true || $jinput->get('option') == 'com_categories')) {
         ...
         $query->join('LEFT', $db->quoteName('#__it_projects').' AS p ON p.id = '.(int) $oldCat);
      ...
      // For new items we want a list of project you are allowed to create in.
      if ($oldCat == 0) {
         ...
         foreach ($options as $i => $option) {
         if ($user->authorise('core.edit.state', $extension . '.itprojects.' . $oldCat) != true && !isset($oldParent)) {
         ...
         // create permission and you should also still be able to save in the current category.
            ...
            if (($user->authorise('core.create', $extension . '.project.' . $option->value) != true)
               && (isset($oldParent)) && $option->value != $oldParent)
            {
               {
                  unset($options[$i]);
