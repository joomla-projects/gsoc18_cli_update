 ...
 */
class IssuetrackerModelIttypes extends JModelAdmin
{
   /**
   ...
   protected $text_prefix = 'COM_ISSUETRACKER';
     ...
     * @since   1.6
     */
   public function getTable($type = 'Ittypes', $prefix = 'IssueTrackerTable', $config = array())
   {
   ...
   public function getForm($data = array(), $loadData = true)
   {
      ...
      // Get the form.
      $form = $this->loadForm('com_issuetracker.ittypes', 'ittypes', array('control' => 'jform', 'load_data' => $loadData));
      if (empty($form)) {
         return false;
      }
   ...
   protected function loadFormData()
   {
      ...
      $data = JFactory::getApplication()->getUserState('com_issuetracker.edit.ittypes.data', array());
   ...
   protected function prepareTable($table)
   {
      ...
      if (empty($table->id)) {
         ...
         if (@$table->ordering === '') {
            $db = JFactory::getDbo();
            $db->setQuery('SELECT MAX(ordering) FROM #__it_types');
            $max = $db->loadResult();
   ...
   public function delete(&$pks)
   {
     ...
     // Check if there are any of the items to be deleted are in use.
      ...
      $query .= ' WHERE i.issue_type IN ( '.implode(',',$pks).' )' ;
      ...
      $row = $this->getTable('ittypes','IssueTrackerTable');
