 ...
 */
class IssueTrackerModelAttachments extends JModelList
{
   ...
   protected function getListQuery()
   {
      ...
      $query->from('`#__it_attachment` AS a');
      ...
      // Join over the foreign key 'issue_id'
      $query->select('iss.issue_summary AS issues_title');
      $query->select('iss.id AS iid');
      $query->join('LEFT', '#__it_issues AS iss ON iss.alias=a.issue_id');
      ...
      if (!empty($search)) {
         if (stripos($search, 'id:') === 0) {
            ...
            $query->where('( a.filename LIKE '.$search.'  OR  a.filetype LIKE '.$search.' )');
         }
      }
